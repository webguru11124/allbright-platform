...
const JobLevelPicker = ({ theme, onChangeText, placeholder, onBlur, error, value }: Props) => {
  ...
  const filteredJobLevels = useMemo(
    () =>
      searchText !== undefined
        ? jobLevels.filter((val) => val.value.toLowerCase().includes(searchText?.toLowerCase()))
        : jobLevels,
    [searchText]
  );
  ...
  const displayValue = useMemo(
    () => (Boolean(value) ? jobLevels.find((item) => item.value === value)?.label : undefined),
    [value]
  );
        ...
        <View style={[styles.centeredView]}>
          <View style={[styles.modalView]}>
            <View style={[styles.titleContainer]}>
              <CM style={[styles.title]}>Job Level</CM>
